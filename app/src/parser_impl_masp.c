...
static parser_error_t readSaplingBundle(parser_context_t *ctx, masp_sapling_bundle_t *bundle) {
    ...
    // Read Value sum
    if (bundle->n_shielded_spends != 0 && bundle->n_shielded_outputs != 0) {
    ...
    if (bundle->n_shielded_outputs != 0 && bundle->n_shielded_spends != 0) {
...
static parser_error_t readSaplingBuilder(parser_context_t *ctx, masp_sapling_builder_t *builder) {
    ...
    CHECK_ERROR(readUint32(ctx, &builder->n_value_sum_asset_type))
